...
public class SystemState
{
    ...
    // 7. Project files — .csproj, .sln, package.json, etc. generated by LLM
    public Dictionary<string, string> ProjectFiles { get; set; } = new();
    // Build config — commands the LLM says are needed to build/run the project
...
public static class Generator
{
    ...
    public static async Task<(Dictionary<string, string> Files, Dictionary<string, string> BuildConfig)> GenerateProjectFiles(SystemState state)
    {
        ...
        var frameworksList = string.Join(", ", state.Frameworks.Select(kv => $"{kv.Key}: {kv.Value}"));
...
public static class PlatinumForgeServer
{
            ...
            if (path == "/api/events" && method == "GET")
            {
                ...
                await sseClient.SendEvent("full-sync", JsonSerializer.Serialize(new
                {
                    state = new {
                        ...
                        iac = live.State.IaC,
                        deployTweaks = live.State.DeployTweaks, projectFiles = live.State.ProjectFiles, buildConfig = live.State.BuildConfig, pipelineConfig = live.State.PipelineConfig,
                    },
                ...
                _ = Task.Run(async () =>
                {
                    try
                    {
                        var stateJson = JsonSerializer.Serialize(new {
                            ...
                            iac = live.State.IaC, deployTweaks = live.State.DeployTweaks, projectFiles = live.State.ProjectFiles, buildConfig = live.State.BuildConfig, pipelineConfig = live.State.PipelineConfig,
                            architectureTweaks = live.State.ArchitectureTweaks,
                            fileManifest = live.State.FileManifest,
    ...
    private static async Task RunGeneration(LiveSession live, string prompt, string userSub = "local")
    {
        ...
        try
        {
            ...
            await BroadcastProgress(live, 0, 11, "Initialising", "running", "Preparing pipeline...");
            ...
            {
                await BroadcastProgress(live, 0, 11, "Interpreting Prompt", "running", "Sending prompt to LLM...");
            ...
            if (StageEnabled(live, "interfaces")) {
                ...
                await BroadcastChat(live, "system", "⏳ [1/11] Generating Interfaces...");
            ...
            if (StageEnabled(live, "unitTests")) {
                ...
                await BroadcastChat(live, "system", "⏳ [2/11] Generating Unit Tests...");
            ...
            if (StageEnabled(live, "code")) {
                ...
                await BroadcastChat(live, "system", "⏳ [3/11] Generating Code...");
            ...
            if (StageEnabled(live, "build") && StageEnabled(live, "unitTests")) {
                ...
                await BroadcastChat(live, "system", "⏳ [4/11] Build & Unit Test loop...");
            ...
            if (unitTestsPassed)
            {
                // Stage 5: Generate Project Files (.csproj, .sln, package.json, etc.)
                string? projectDir = null;
                Process? appProcess = null;
                string? appUrl = null;
                try
                {
                if (StageEnabled(live, "projectBuild")) {
                    await BroadcastProgress(live, 5, 11, "Project Files", "running", "Generating project/build files...");
                    await BroadcastChat(live, "system", "⏳ [5/11] Generating project files...");
                    ...
                    await BroadcastChat(live, "system", "⏳ [6/11] Building project on disk...");
                    projectDir = Path.Combine(Path.GetTempPath(), $"platinumforge-project-{Guid.NewGuid():N}");
                    Directory.CreateDirectory(projectDir);

                    // Write project files (.csproj, .sln, etc.)
                    ...
                    // Write code files (path-prefixed)
                    ...
                    if (!string.IsNullOrWhiteSpace(installCmd))
                    {
                        ...
                        {
                            await BroadcastChat(live, "system", "✅ Dependencies installed");
                        }
                    }
                    ...
                    {
                        await BroadcastProgress(live, 6, 11, "Project Build", "done", "No build command");
                    }
                } else {
                ...
                if (StageEnabled(live, "appVerify") && projectDir != null) {
                    var runCmd = live.State.BuildConfig.GetValueOrDefault("runCommand", "");
                    var healthUrl = live.State.BuildConfig.GetValueOrDefault("healthCheckUrl", "");
                    if (!string.IsNullOrWhiteSpace(runCmd))
                    {
                        await BroadcastProgress(live, 7, 11, "App Verify", "running", "Launching application...");
                        await BroadcastChat(live, "system", $"⏳ [7/11] Launching app: {runCmd}");

                        appProcess = LaunchApp(runCmd, projectDir);
                        appUrl = healthUrl;
                        ...
                        if (appProcess != null && !string.IsNullOrWhiteSpace(healthUrl))
                        {
                            ...
                            if (healthy)
                            {
                                ...
                                await BroadcastProgress(live, 7, 11, "App Verify", "done", "App responding");
                            }
                            else
                            {
                        ...
                        else
                        {
                            ...
                            {
                                await BroadcastChat(live, "success", "✅ App process started (no health URL to verify)");
                                await BroadcastProgress(live, 7, 11, "App Verify", "done", "Process running");
                            }
                            else
                            {
                                await BroadcastChat(live, "error", "❌ App process exited immediately");
                                await BroadcastProgress(live, 7, 11, "App Verify", "fail", "Process crashed");
                            }
                        }
                    }
                    else
                    {
                        await BroadcastProgress(live, 7, 11, "App Verify", "done", "No run command");
                    }
                } else {
                ...
                if (StageEnabled(live, "iac") && (live.State.Deployment.Count > 0 || live.State.IaC.Count > 0))
                {
                    ...
                    await BroadcastChat(live, "system", "⏳ Generating Infrastructure as Code...");
                ...
                {
                    await BroadcastProgress(live, 10, 11, "Infrastructure", "done", "Skipped — no deployment config");
                }
                ...
                if (StageEnabled(live, "publish")) {
                    ...
                    await BroadcastChat(live, "system", "⏳ [11/11] Publishing artifact...");
            ...
            {
                await BroadcastProgress(live, 4, 11, "Build & Test", "fail", "Pipeline halted");
                await BroadcastChat(live, "error", "⚠️ Unit tests failed — skipping remaining stages");
            }
        }
        catch (Exception ex)
        {
            await BroadcastProgress(live, 0, 11, "Error", "fail", ex.Message);
            await BroadcastChat(live, "error", $"❌ Generation failed: {ex.Message}");
        }
        finally
        {
            var elapsed = (DateTime.UtcNow - pipelineStart).TotalSeconds;
            await BroadcastProgress(live, 11, 11, "Complete", "complete", $"Total: {elapsed:F1}s");
        ...
        <body>
            ...
            <script>
                ...
                const PIPELINE_STAGE_META = {
                    ...
                    build:            { label: 'Build & Test Loop',       icon: '🔨', desc: 'Compile and run unit tests with retry (Roslyn)' },
                    projectBuild:     { label: 'Project Build',           icon: '🏗️', desc: 'Generate .csproj/.sln, write to disk, dotnet build' },
                    appVerify:        { label: 'App Verification',        icon: '🚀', desc: 'Launch app with dotnet run and health-check' },
                    nfrTests:         { label: 'NFR Tests (Playwright)',   icon: '🎭', desc: 'Generate and run browser/UI tests against app' },
                    soakTests:        { label: 'Soak Tests (Locust)',     icon: '🌊', desc: 'Generate and run load/performance tests against app' },
                    integrationTests: { label: 'Integration Tests (Jest)', icon: '🔗', desc: 'Generate and run integration tests against app' },
